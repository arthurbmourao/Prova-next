...
export default function CardCurso({card} : CardCursoProps){
    ...
    //)
//
    //const dinheiro = card.preco;
    //const real = dinheiro.toLocaleString("PT-br",{style: "currency"})
    ...
    return (
        <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
          ...
          />
          <div className="p-5">
            {`${card.destaque ? '<span className="inline-flex rounded-full bg-emerald-50 px-3 py-1 text-xs font-extrabold uppercase tracking-wide text-emerald-700">Destaque</span>' : '<span class="inline-flex rounded-full bg-weg-light px-3 py-1 text-xs font-extrabold uppercase tracking-wide text-weg-dark">Curso</span>'}`}
            ...
            <div className="mt-4 flex flex-wrap gap-2 text-xs font-bold text-slate-600">
              ...
              {(card.vagas<10) ? '<span className="rounded-full bg-slate-100 px-3 py-1">{card.vagas} vagas</span>' : '<span className="rounded-full bg-slate-100 px-3 py-1">{card.vagas} vagas</span>'}
